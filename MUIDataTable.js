...
class MUIDataTable extends React.Component {
  ...
  setTableData(props, status, callback = () => {}) {
    let tableData = [];
    const {columns, filterData, filterList} = this.buildColumns(props.columns);
    ...
    const displayData = this.getDisplayData(columns, tableData, filterList, searchText);
  ...
  selectRowDelete = () => {
    ...
    if (this.options.onRowsDelete) {
      if (this.options.onRowsDelete(selectedRows) === false) {
        return;
      }
    }
  ...
  toggleExpandRow = (row) => {
    const {dataIndex} = row;
    const expandedRows = [...this.state.expandedRows.data];
    let rowPos = -1;
  ...
  render() {
    ...
    return (
      ...
      >
        {typeof this.options.toolbar === 'undefined' || this.options.toolbar ?
          <TableToolbar
            columns={columns}
            data={data}
            displayData={displayData}
